...
// Submit nappi pois käytöstä jos peli ei oo alkanut vielä
...
function startGame() {
    ...
    // Heiton submit nappi käyttöön kun peli alotettu
...
function throwDart() {
    ...
    // Jos legi päättynyt, viesti siitä tyhjäks kun heitetään seuraavaks
    if (legOverMessage.textContent) {
        legOverMessage.textContent = "";
    }

    // Kaikki luvut 0-60 väliltä ei oo mahollisia, lista mahollisista pisteistä
    ...
    // Jos ei bustannu, heitetyt pisteet näkyviin
    ...
    if (currentPlayer.points === 0) {
        ...
        if (currentPlayer.legsWon === winningLegs) {
            ...
            // Peli päättyy, submit nappi taas pois käytöstä
            submitScoreButton.disabled = true;
            return;
        }
        // Legivoitto
    ...
    // Vuoro vaihtuu kolmen heiton jälkeen
    if (throwsThisTurn === 3) {
        throwsThisTurn = 0;
        currentPlayer = (currentPlayer === player1) ? player2 : player1;
        turnMessage.textContent = "Current turn: " + currentPlayer.name  + ", throws: " + throwsThisTurn;
